...
def execute_engine(file_path):
    if file_path:
        ...
        result = subprocess.run(["D:\\OS project\\dist\\TotalProtect.exe", file_path], stdout=subprocess.PIPE)
...
my_label = tb.Label(text="Antivirus", font=("Helvetica", 40), bootstyle="default")
...
image = PhotoImage(file="fileLogo.png")

# Create a label with the image
image_button1 = tb.Label(image=image)
image_button1.pack(pady=10)

# Bind the click event of the image to the file_dialog function
image_button1.bind("<Button-1>", lambda event: file_dialog())
